package net.uweeisele.pseudonymize.distributed;

import net.uweeisele.account.v1.Account;
import net.uweeisele.account.v1.CreatePseudonymAccountCommand;
import net.uweeisele.account.v1.PseudonymAccountEvent;
import net.uweeisele.action.v1.ActionEvent;
import io.confluent.kafka.streams.serdes.avro.SpecificAvroSerde;
import net.uweeisele.support.*;
    ...
    public StreamsApp runApp(final AppConfigs appConfigs) {
        ...
        final AppConfigs actualAppConfigs = appConfigs.createAppConfigsWith(newConfigs ->
            newConfigs.computeIfAbsent(STATE_DIR_CONFIG,
                    key -> FileSupport.deleteCloseable(FileSupport.tempDirectory("kafka-streams"), closeables::add).getPath())
        );
